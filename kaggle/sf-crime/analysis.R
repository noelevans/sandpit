...
train <- feature_engineering(train_filename, TRUE)
test <- feature_engineering(test_filename)
...
for (d in districts) {
    ...
    print(paste("Growing forest for", d))
    fit <- randomForest(formula, data=district_train, ntrees=n_trees)
