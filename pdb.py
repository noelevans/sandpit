...
class Pdb(pdb.Pdb, ConfigurableClass, object):
    ...
    def _is_hidden(self, frame):
        ...
        if frame.f_globals.get("__unittest"):
            return True
        if frame.f_locals.get("__tracebackhide__") or frame.f_globals.get(
            "__tracebackhide__"
        ):
            return True
    ...
    def _init_pygments(self):
        ...
        if hasattr(self.config, "formatter"):
            ...
            if self.config.use_terminal256formatter or (
                self.config.use_terminal256formatter is None
            ...
            self._fmt = Formatter(
                bg=self.config.bg, colorscheme=self.config.colorscheme
            )
